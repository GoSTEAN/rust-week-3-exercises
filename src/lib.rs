...
impl<'de> Deserialize<'de> for Txid {
        ...
        D: serde::Deserializer<'de>,
    {
        ...
        if bytes.len() != 32 {
            return Err(D::Error::custom("Txid must be 32 bytes"));
        }
        let mut txid = [0u8; 32];
...
impl Script {
    ...
    pub fn from_bytes(bytes: &[u8]) -> Result<(Self, usize), BitcoinError> {
        ...
        let len = compact_size.value as usize;
...
impl TransactionInput {
    ...
    pub fn from_bytes(bytes: &[u8]) -> Result<(Self, usize), BitcoinError> {
        ...
        if bytes.len() < outpoint_consumed + script_consumed + 4 {
            return Err(BitcoinError::InsufficientBytes);
        }
        let sequence = u32::from_le_bytes([
            bytes[outpoint_consumed + script_consumed],
            bytes[outpoint_consumed + script_consumed + 1],
            bytes[outpoint_consumed + script_consumed + 2],
            bytes[outpoint_consumed + script_consumed + 3],
        ]);
        Ok((
            TransactionInput {
            ...
            outpoint_consumed + script_consumed + 4,
        ))
    }
}
...
impl BitcoinTransaction {
    ...
    pub fn from_bytes(bytes: &[u8]) -> Result<(Self, usize), BitcoinError> {
        ...
        let (input_count, count_consumed) = CompactSize::from_bytes(&bytes[4..])?;
        let mut inputs = Vec::new();
        let mut offset = 4 + count_consumed;
